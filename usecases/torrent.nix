...
in {
  ...
  systemd.services.sonarr_radarr_rights = {
    description = "Allow access to users in the sonarr / radarr groups";
    after = [ "sonarr.service" "radarr.service" ];
    # Restart of sonarr or radarr are restarted
    partOf = [ "sonarr.service" "radarr.service" ];
    wantedBy = [ "multi-user.target" ];

    script = ''
      ${pkgs.coreutils}/bin/chmod 770 /var/lib/sonarr/ /var/lib/radarr/
    '';
